...
```{r}
c("a", as.raw(1)); c(1.0, as.raw(1)); 
c(1L, as.raw(1)); c(TRUE, as.raw(1))
```
...
# rsmf型をdata.frameに変換。音の高さやなる時間などを抜き出す
...
ggplot(nf)+geom_segment(aes(x=start_time,y=height,
                            xend=end_time,yend=height))+
  xlab("time")+ylab("height")+xlim(c(0,10000))
```
...
## ありがとうぞうさん
* このスライドはKazutan.Rの各種資料とcssファイルを参考にさせていただました。ありがとうございました。
